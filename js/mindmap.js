        ...
        function createMiniEditPropertiesPanel(node, textarea, screenX, screenY, screenW) {
            ...
            panel.style.left = `${Math.max(8, Math.min(window.innerWidth - 310, screenX))}px`;
            panel.style.top = `${Math.max(8, screenY - 44)}px`;
            panel.innerHTML = `
            ...
            panel.innerHTML = `
                ...
                <button type="button" data-format="**" title="Bold"><b>B</b></button>
                <button type="button" data-format="__" title="Italic"><i>I</i></button>
                <button type="button" data-format="++" title="Underline (Ctrl/Cmd+U)"><u>U</u></button>
                <button type="button" class="node-mini-text-swatch" data-palette="text" title="Text color"><u>A</u></button>
                <button type="button" data-more title="More properties">•••</button>
                <div class="node-mini-more" hidden>
                    <button type="button" data-font="-" title="Smaller text">A−</button>
                    <button type="button" data-font="+" title="Larger text">A+</button>
                    <button type="button" data-align="left" title="Align left (Ctrl/Cmd+L)"><svg viewBox="0 0 24 24"><path d="M4 5h16M4 10h10M4 15h16M4 20h10"/></svg></button>
                    <button type="button" data-align="center" title="Align center (Ctrl/Cmd+E)"><svg viewBox="0 0 24 24"><path d="M4 5h16M7 10h10M4 15h16M7 20h10"/></svg></button>
                    <button type="button" data-align="right" title="Align right (Ctrl/Cmd+R)"><svg viewBox="0 0 24 24"><path d="M4 5h16M10 10h10M4 15h16M10 20h10"/></svg></button>
                    <button type="button" data-format="~~" title="Strikethrough"><s>S</s></button>
                    <button type="button" data-format="==" title="Highlight">H</button>
            ...
            panel.querySelectorAll('button[title]').forEach(button => button.setAttribute('aria-label', button.title));
        ...
        function startInlineEdit(nodeId) {
            ...
            const textarea = window.createMindmapRichEditor(node);
            ...
            textarea.addEventListener('keydown', e => {
                if ((e.ctrlKey || e.metaKey) && ['b', 'i', 'u'].includes(e.key.toLowerCase())) {
                    e.preventDefault(); e.stopPropagation(); applySelectedTextFormat(node, textarea, { b: '**', i: '__', u: '++' }[e.key.toLowerCase()]); return;
                }
                if ((e.ctrlKey || e.metaKey) && ['l', 'e', 'r'].includes(e.key.toLowerCase())) {
                    e.preventDefault(); e.stopPropagation(); node.textAlign = { l: 'left', e: 'center', r: 'right' }[e.key.toLowerCase()]; render(); return;
                }
