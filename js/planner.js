(() => {
    ...
    window.setupWeeklyPlanner = (dashboard) => {
        ...
        const renderDaily = () => {
            ...
            window.renderPlannerMatrix();
            renderList('[data-scheduled]', state.items.filter(item => M.segments([item], selectedDate).length), false);
            renderList('[data-recurring]', state.recurring.filter(item => M.segments([item], selectedDate).length), true);
        };
        ...
        const renderCalendar = () => {
            ...
            for (let index = 0; index < cells; index++) {
                ...
                const entries = M.layout(M.segments([...state.items, ...state.recurring], value));
                ...
                (isWeek ? entries : entries.slice(0, 2)).forEach(entry => {
                    ...
                    eventRow.addEventListener('dragstart', event => { event.stopPropagation(); event.dataTransfer.setData('application/x-hodi-calendar', JSON.stringify({ id: item.id, date: occurrenceDate })); event.dataTransfer.effectAllowed = 'move'; });
                ...
                cell.addEventListener('drop', e => {
                    ...
                    if (payload) {
                        ...
                        if (!item) return;
                        if (dropMinute !== null) { calendarEditor.changeTime(item, moved.date, value, dropMinute, M.duration(item) || 60); return; }
