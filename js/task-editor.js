(() => {
    ...
    window.editDashboardTask = (task, save) => {
        ...
        preview(); form.elements.categoryColor.oninput = preview;
        const hideOptions = () => { options.hidden = true; form.querySelector('[data-category-menu]').setAttribute('aria-expanded', 'false'); };
        categories.forEach(value => { const option = document.createElement('button'); option.type = 'button'; option.textContent = value; option.onclick = () => { form.elements.category.value = value; hideOptions(); }; options.appendChild(option); });
        ...
        form.querySelector('[data-new-category]').onclick = () => { hideOptions(); form.elements.category.value = ''; form.elements.category.focus(); };
        const previous = document.activeElement;
